...
from cliflix.search.helpers import retrieve_url
...
class limetorrents(object):
    ...
    class MyHtmlParser(HTMLParser):
        ...
        def handle_starttag(self, tag, attrs):
            ...
            if tag == self.TR and self.findTable and (
                    params.get('bgcolor') == '#F4F4F4' or params.get('bgcolor') == '#FFFFFF'):  # noqa
                self.inside_tr = True
                self.current_item = {}
        ...
        def handle_endtag(self, tag):
            ...
            if self.inside_tr and tag == self.TR:
                ...
                if array_length < 1:
                    return
                self.all_items.append(self.current_item)
    ...
    def search(self, query, cat='all'):
        ...
        while True:
            ...
            if page > 1 or lunghezza_html <= parser.page_empty:
                break
            parser.feed(html)
            page += 1

        parser.close()
        for i in range(len(parser.all_items))[:5]:
            parser.all_items[i]["link"] = self.download_torrent(parser.all_items[i]["desc_link"])
        return parser.all_items[:5]
