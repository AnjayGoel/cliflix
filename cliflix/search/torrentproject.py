    ...
    def search(self, what, cat='all'):
        ...
        for currPage in range(0, 10):
            ...
            if len(parser.pageRes) <= 0:
                break
            del parser.pageRes[:]
        parser.fullResData = sorted(parser.fullResData, key=lambda i: int(i['seeds']))[::-1]
        for i in range(len(parser.fullResData))[:5]:
            torr = parser.fullResData[i]
            parser.fullResData[i]["link"] = self.download_torrent(torr["desc_link"])
        parser.close()
        return parser.fullResData[:5]
